...
class ReportsController extends Controller
{
    ...
    public function attendance_report(Request $request)
    {
        ...
        if ($button == "export") {
            ...
            $export = new ExcelExport($exportExcel, $headers);
            return Excel::download($export, 'attendance_report_' . $year . '_' . $month . '.xlsx');
        }

        return view('backend.reports.attendance_report', compact('attendances', 'headers', 'year', 'month', 'departmentId', 'searchEmp', 'search', 'month_year'));
    }
    ...
    public function accruals_report(Request $request)
    {
        ...
        foreach ($employees as $row => $employee) {
            try {
                ...
                $accruals[$row]["department_name"] = $employee->department->name;
                $accruals[$row]["designation_name"] = $employee->designation->name;
                $basic = 0;
                $hra = 0;
                $travel_allowance = 0;
                $other_allowance = 0;
    ...
    public function generateBranchBudgetReport(Request $request)
    {
        ...
        foreach ($departments as $department) {
            ...
            $row = [
                ...
                'branch_budget' => $department->budget > 0 ? $department->budget . ' ' . getSetting('currency') : '0.00',
            ];
            ...
            $row['total_spent'] = number_format($total_user_salary, 2) > 0 ? number_format($total_user_salary, 2) . ' ' . getSetting('currency') : '0.00';
            $row['status'] = $is_over_budget;
            $row['amount'] = abs($total_user_salary - $department->budget) > 0 ? abs($total_user_salary - $department->budget) . ' ' . getSetting('currency') : '0.00';
        ...
        if ($request->ajax()) {
            ...
            foreach ($users as $user) {
                ...
                $policymonth = match ($workdetails->renewal_air_ticket) {
                    '1_year' => 12,
                    '2_year' => 24,
                    default => $airtickeCountrySetting->request_after_months
                        ?? $airtickeSetting->request_after_months
                        ?? 0,
                };
    ...
    public function calculateVacationBalance(User $user, LeaveType $leaveType)
    {
        ...
        if ($isMonthWise) {
            ...
            while ($monthPointer->lte($today)) {
                ...
                if ($monthPointer->lt($sixMonthDate) &&  $monthwise2leave == 1 && $isBeforeSixMonth) {
                ...
                if ($monthPointer->isSameMonth($oneYearDate->copy()->subMonth())) {
            ...
            while ($monthPointer->lte($today)) {
                ...
                if ($monthPointer->lt($oneYearDate) && $yearGiven2Leave == 1) {
                    $leaveForMonth = 2;
                ...
                if ($monthPointer->isSameMonth($oneYearDate->copy()->startOfMonth()))
                    // 🟢 CASE 4: On 2nd year 1st month (after completing 1 year) → full yearly leave credit
                    if (
                        $monthPointer->isSameMonth($oneYearDate->copy()->startOfMonth())
                    ) {
                ...
                if ($monthPointer->gt($oneYearDate->copy()->endOfMonth())) {
                    $leaveForMonth = 0;
