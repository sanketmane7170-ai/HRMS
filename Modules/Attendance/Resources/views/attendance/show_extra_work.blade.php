    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">{{__trans('user_extra_hours')}}</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{route('backend.dashboard')}}">{{__trans('dashboard')}}</a>
                        </li>
                        <li class="breadcrumb-item active">{{__trans('user_extra_hours')}}</li>
                    </ul>
                </div>
                <div class="col-auto">
                    @if (hasPermission('Create Over Time Request'))
                        <a href="{{route('backend.addEmpExtraHours')}}" class="btn btn-primary me-1 edit-button">
                            <i class="fa fa-plus"></i>
                        </a>
                    @endif
                    @if (hasPermission('Manage Over Time Request'))
                        <a href="{{route('backend.showExtraHoursReport')}}" class="btn btn-primary me-1">
                            <i class="fas fa-info"></i> {{__trans('show extra hours report')}}
                        </a>
                    @endif
                    @if (hasPermission('Manage Over Time Request'))
                    ...
                    @if (hasPermission('Manage Over Time Request'))
                        <select name="take_actions" class="form-control select2" id="takeActions" style="display: none;">
                            ...
                            <option value="1">Add To Payroll</option>
                            <option value="2">Add To Leave</option>
                            ...
                            <table class="table text-center table-hover" id="dataTable">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th>{{ __trans('added_by') }}</th>
                                        <th>{{ __trans('extra_hours') }} (HH:MM)</th>
                                        <th>{{ __trans('Payable Hours') }}</th>
                                        <th>{{ __trans('date') }}</th>
...
<script type="text/javascript">
    $('#dataTable').DataTable({
        ...
        columns: [
            ...
            { data: 'extra_hours', name: 'extra_hours', orderable: false, searchable: false  },
    ...
    document.getElementById('selected_month_input').addEventListener('change', function () {
        ...
        let newUrl = "{{ route('backend.extra.show') }}";
