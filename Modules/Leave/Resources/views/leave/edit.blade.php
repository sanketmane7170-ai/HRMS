<div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title">{{__trans('edit_leave_request')}} - ({{$user->employee_id}}) {{$user->name}}</h4>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <form action="{{route('backend.leaves.update',$leave)}}" datatable="true" method="POST" class="ajax-form-submit reset">
            @csrf
            @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select name="leave_type_id" id="leave_type_id" class="select-search" @if($leave->status->value == 'approved') disabled @endif>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label for="is_half_day">{{__trans('is_half_day_leave')}}
                                <br>
                                <input type="checkbox" @if($leave->is_half_day) value="1" @else value="0" @endif class="form-check-input" name="is_half_day" id="is_half_day" @if($leave->is_half_day) checked @endif>
                            </label>
                        </div>
                    </div>
                    <div class="col-md-12">
