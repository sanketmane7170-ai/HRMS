...
class UserLeaveBalance extends Component
{
    ...
    {
        $this->user = $user;
        $this->types = LeaveType::get(['id', 'name', 'days']);
    }
    ...
    {
        return view('leave::components.userleavebalance');
    }

    // public function calculatePendingLeave(LeaveType $leaveType)
    // {
    ...
    //     $joining_date = Carbon::parse($this->user->workDetail?->joining_date);
    ...
    //                         if($checkLeaveBalance->available >= $leaveType->no_of_leaves){
    //                             $added_day = $leaveType->no_of_leaves;
    //                         } else {
    ...
    //     $joining_date = $this->user->workDetail?->joining_date->toDateString();
    //     // joining date
    //     if($daysDiff <= 365){
    ...
    //                 $isCheckin = Attendance::where('user_id', $this->user->id)
    ...
    //         $extraWorkAddToLeaves = extraWorkRequest::where([['user_id',$this->user->id],['status',2]])->where('year', $currentYear)->get();
    //         $availableDay = 0;
    ...
    //         // $usershift = UsersShift::where('user_id', $this->user->id)->whereBetween('assigned_for_date', [$startDate, $endDate])->get();
    //         $usershift = UsersShift::where('user_id', $this->user->id)
    ...
    //         $probationDate = UserWorkDetail::where('user_id',$this->user->id)->first();
    ...
    //         if($updatebal){
    //             if($updatebal->leave_type_id==$leaveType->id){
    ...
    //                         $availableDay = $diff;
    //                     }
    //                     if($checkLeaveBalance->available != $availableDay){
    //                         $logupdate = leaveLog::create([
    //                             'user_id' => $this->user->id,
    //                             'leave_type_id' => $leaveType->id,
    //                             'previous_balance' => $checkLeaveBalance->available,
    //                             'new_balance' =>  $availableDay,
    //                             'updated_by' => auth()->user()->id,
    //                             'updated_at' => now(),
    //                             'description' => 'this updated by '.auth()->user()->name.' in user leave balance model fun.'
    //                         ]);
    //                     }
    ...
    //                         'available' => $availableDay,
    //                         'isAddThisMonthLeave' => date('m'),
    //                         'thisYearAvailableLeave' => $total_days - $total_approved_leaves
    //                     ]
    //                 );
    //             } else {
    //                 // previous leave balance
    ...
    //                .'Total Balance From DB'.$balance->available);
    //     return $balance->available;
    // }

    public function progressBarPercentage(int $available, LeaveType $leaveType)
    {
        $total_days = $leaveType->days;
        $oneYearBack = Carbon::now()->subYear()->year;
        $new_days = 0;
        ...
        // if($leaveType->is_recurring == '1'){
            $currentYearDate = Carbon::now()->format('d-m-Y');
            $startYear = '01-01-'.date('Y');

            if($currentYearDate == $startYear){
                $previous_year_leave = PreviousYearLeave::where([
                    ['user_id',$this->user->id],
                    ['year', $oneYearBack],
                    ['leave_type_id',$leaveType->id],
        ...
        // }
        // if($leaveType->is_recurring == '1'){
        //     $last_year_balance = LeaveBalance::where(['leave_type_id'=>$leaveType->id,'user_id'=>$this->user->id,'year'=>$oneYearBack])->value('available');
        ...
        $percenatge = 0;
        if ($total_days > 0) {
            $percenatge = (($available / $total_days) * 100);
        }


        return ceil($percenatge);
    }

    public function getURLParamsForLeave(LeaveType $leaveType){
        $result = [
            'user_id' => $this->user->id,
            'leave_type_id' => $leaveType->id
        ];
    ...
    public function calculatePendingLeave(LeaveType $leaveType)
    {
        ...
        if(!$balance){
            $userfromtable = User::find($this->user->id);
            $user_id = $userfromtable->id;
            if(!isset($userfromtable->workDetail?->joining_date)) {
                return 0;
            }
        ...
        \Log::info('Total Days'.$availableBalance.' '.'Total Balance From DB'.$availableBalance);
