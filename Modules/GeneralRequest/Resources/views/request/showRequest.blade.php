    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">{{__trans('general_request_list')}}</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{route('backend.dashboard')}}">{{__trans('dashboard')}}</a>
                        </li>
                        <li class="breadcrumb-item active">{{__trans('general_request_list')}}</li>
                    </ul>
                </div>
                <div class="col-auto">
                    <a href="{{route('backend.admin.generalRequest.create')}}" class="btn btn-primary me-1 edit-button">
                        <i class="fas fa-plus"></i> {{__trans('add_general_request')}}
                    </a>
                    <button type="button" class="btn btn-primary dark">
                        <a href="{{ route('backend.generalRequest.generalRequestTransaction') }}" style="color: white;">
                            ...
                            <table class="table text-center table-hover" id="dataTable">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th>{{__trans('general_request_type')}}</th>
                                        <th>{{__trans('date')}}</th>
...
<script type="text/javascript">
    var table = $('#dataTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{route('backend.admin.generalRequest')}}",
        },
        order: [[0, 'desc']],
        columns: [{
                data: 'DT_RowIndex',
