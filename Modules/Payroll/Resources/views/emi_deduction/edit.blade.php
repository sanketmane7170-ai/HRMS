<div class="modal-dialog modal-lg" style="max-width: 523px !important;">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title">@if(request()->getHttpHost()=="cakesocial.momdigital.io"){{__trans('add_employee_deductions')}} @else{{__trans('add_employee_deduction')}} @endif  : {{$user->name}}</h4>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <form action="{{route('backend.payroll.updateEMIDeduction', $emi_deduction->id)}}" datatable="true" method="POST" class="ajax-form-submit reset">
            @csrf
                ...
                <div class="row">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="deduction" class="form-control select-search" id="deduction">
                                ...
                                @foreach ($fixedDeduction as $item)
                                    <option value="{{ $item->id }}" @if($item->id == $emi_deduction->deduction_id) selected @endif>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <input type="text" step="0.01" name="total_amount" value="{{ $emi_deduction->total_amount }}" class="form-control" id="total_amount" placeholder="{{__trans('total_amount')}}">
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="mb-3">
                            <label for="remark" class="form-label">{{__trans('remark')}}</label>
                            <textarea name="remark" class="form-control" id="remark" placeholder="{{__trans('remark')}}">{{ $emi_deduction->remark }}</textarea>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div id="monthAmountWrapper">
                            @foreach ($emiDeductionData as $item)
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-info waves-effect waves-light save-emi-deduction">{{__trans('update')}} </button>
            </div>
        </form>
    </div>
</div>
