@extends('layouts.backend')
@push('css')
<link rel="stylesheet" href="{{asset('assets/backend/plugins/flatpickr/flatpickr.min.css')}}">
@endpush
@section('content')

<!-- Page Wrapper -->
<div class="page-wrapper">
    <div class="content container-fluid">
        <!-- Page Header -->
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">{{__trans('shift_roster')}}</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{route('backend.dashboard')}}">{{__trans('dashboard')}}</a>
                        </li>
                        <li class="breadcrumb-item active">{{__trans('shift_scheduler')}}</li>
                    </ul>
                </div>
            </div>
        </div>
        <form action="{{route('backend.shift.roster.search')}}" enctype="multipart/form-data" method="POST">
            @csrf
            <div class="row">
                <div class="col-xl-6 col-12">
                    <div class="card bg-white">
                        <div class="card-body">
                            <div class="row" id="custom_container">
                                <div class="col-lg-5">
                                    <div class="form-group">
                                        <label>{{__trans('department')}}</label>
                                        <select name="department_id" id="department" class="form-control select">
                                            {{-- <option value="{{$user->department->id}}">{{$user->department?->name ?? 'NA'}}</option> --}}
                                            <option value="">{{__trans('select_a_option')}}</option>
                                            <option value="0">{{__trans('all')}}</option>
                                                @foreach (\App\Models\Department::all() as $department)
                                                    <option @if($department->id == $departmentId) selected @endif value="{{$department->id}}">{{$department->name}}</option>
                                                @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                                    <div class="form-group">
                                        <label>{{__trans('start_date')}}</label>
                                        <input type="text" class="form-control datepicker" name="start_date" value="{{$startDate}}">
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="mb-3" style="margin-top: 31px !important;">
                                        <button type="submit" id="searchRoster" class="btn btn-primary">
                                            <i class="fa fa-search mr-2" style="display: inline"></i>Search
                                        </button>
                                        @if ($departmentId)
                                            <a href="{{route('backend.shift.roster.print', [$departmentId, $startDate])}}" class="btn btn-sm btn-success mt-2">
                                                <i class="fa fa-file-excel mr-2" style="display: inline"></i>Export
                                            </a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </form>
        <!-- /Page Header -->
        @if ($users)
            <div class="row">
                <div class="col-sm-12">
                    <div class="card card-table">
                        <div class="card-body">

                            <table class="table table-bordered table-hover">
                                <thead>
                                    <tr>
                                    <th scope="col" style="background-color: #042356;" class="text-white">
                                        <strong>Employee Name</strong>
                                    </th>
                                        @foreach($dates as $date)
                                            <th class="text-center text-white" style="background-color: #042356">
                                                <strong>
                                                    @php
                                                        $newDate = Carbon\Carbon::createFromFormat('Y-m-d', $date);
                                                    @endphp
                                                    <small><strong>{{substr($newDate->format('l'), 0,3)}} {{ $newDate->format('d-m-Y') }}</strong></small>
                                                </strong>
                                            </th>
                                        @endforeach
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($users as $user)
                                        <tr>
                                            <th style="background-color: #042356" class="text-white">
                                                <strong>{{ Str::limit($user->name, 20) }}</strong><br>
                                                <small>({{ $user->employee_id }})</small>
                                            </th>
                                            @foreach($dates as $date)
                                                <td>
                                                    @foreach($user->assigned_shifts as $shift)
                                                        @if($shift->assigned_for_date === $date)
                                                            <div class="">
                                                                <span class="badge badge-primary mb-2">{{ $shift->shift_schedule_information->title }}<br>
                                                                    <small>{{ substr($shift->shift_schedule_information->shift_start,0,5) }} - {{ substr($shift->shift_schedule_information->shift_end,0,5) }}</small>
                                                                </span>
                                                                <br>

                                                            <div>
                                                        @endif
                                                    @endforeach
                                                </td>
                                            @endforeach
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        @endif
    </div>
</div>

@endsection

@push('scripts')
<script src="{{asset('assets/backend/plugins/flatpickr/flatpickr.min.js')}}"></script>
<script>

    flatpickr("input.datepicker", {
        dateFormat: "Y-m-d",
    });
</script>
@endpush
